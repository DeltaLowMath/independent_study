...
AShooterCharacter::AShooterCharacter() :
	...
	CameraInterpDistance(128.f),
	CameraInterpElevation(64.f),
	...
	EquipSoundResetTime(0.2f)
{
	...
	// Create a camera boom (pulls into towards character if there is a collision)
	...
	// Do not rotate when the controller rotates, let controller only affect the camera
	...
	InterpComp0 = CreateDefaultSubobject<USceneComponent>(TEXT("Interpolation Component 0"));
	InterpComp0->SetupAttachment(GetFollowCamera());
...
bool AShooterCharacter::GetBeamEndLocation(const FVector& BarrelSocketLocation, FVector& OutBeamLocation)
{
	...
	if (bCrosshairHit)
	{
		//Tentative beam loacation, still need to trace from gun
	...
	// Trace outward from barrel position
...
void AShooterCharacter::CalculateCrosshairSpread(float DeltaTime)
{
	...
	{
		// Shrink the crosshairs rapid while on the ground
	...
	if (bShouldTraceForItems)
	{
		...
		if (ItemTraceResult.bBlockingHit)
		{
			...
			if (TraceHitItemLastFrame)
			{
				...
				{
					// Hitting a different AItem this frame from last frame or AItem is null
...
bool AShooterCharacter::bWeaponHasAmmo()
{
...
void AShooterCharacter::ReloadWeapon()
{
	...
	if (bCarryingAmmo() && !EquippedWeapon->bClipIsFull())
	{
		if (bAiming)
		{
			StopAiming();
...
bool AShooterCharacter::bCarryingAmmo()
{
	if (EquippedWeapon == nullptr) return false;
...
void AShooterCharacter::InitializeInterpLocations()
{
	...
	FInterpLocation InterpLoc0{ InterpComp0, 0 };
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType))
	{
		...
		// Space left inthe magazine of EquippedWeapon
